...
def main_app():
    ...
    def parse_ai_response(text, is_video=False, is_vector=False):
        ...
        if is_video: category = "Videos"
        elif is_vector: category = "Vectors"
        else: category = "Illustrations/Clip Art"
            
        return title, keywords_str, category

    def get_available_gemini_models(api_key):
        if not HAS_GEMINI: return ["gemini-1.5-flash"]
        try:
            ...
            return res if res else ["gemini-1.5-flash", "gemini-2.0-flash"]
        except Exception:
    ...
    if uploaded_files:
        ...
        with st.expander("📋 รายชื่อไฟล์ที่เตรียมประมวลผล", expanded=True):
            for idx, file in enumerate(uploaded_files):
                ...
                col_name.write(f"**[{idx+1}]** `{file.name}`")
                status_placeholders[file.name] = col_status.empty()
            ...
            if "Gemini" in api_choice and not gemini_api_key:
                ...
                for idx, file in enumerate(uploaded_files):
                    if file.name in status_placeholders:
                        status_placeholders[file.name].info("🔄 กำลังอ่านไฟล์...")
                    try:
                        if "Gemini" in api_choice:
